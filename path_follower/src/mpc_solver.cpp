...
inline double sabs(double x) { return std::sqrt(x * x + PWR_EPS2); }
...
inline double predict_power(double v, double w, double a, double alpha) {
    return PWR_C[0]
        + PWR_C[1] * v * a + PWR_C[2] * w * alpha
        + PWR_C[3] * a * a + PWR_C[4] * alpha * alpha
        + PWR_C[5] * sabs(v) + PWR_C[6] * sabs(w)
        + PWR_C[7] * v * v + PWR_C[8] * w * w
        + PWR_C[9] * sabs(a) + PWR_C[10] * sabs(alpha)
        + PWR_C[11] * sabs(v * w);
}

inline double clamp_prev_cmd(
    double cmd_prev, double status, double cmd_act_diff_max,
...
double advance_u_progress(
    double u_cur, const StateVec& x,
...
inline void eval_bspline2(
    const std::vector<Eigen::Vector2d>& cps, double u_in,
    Eigen::Vector2d* p, Eigen::Vector2d* d1, Eigen::Vector2d* d2
) {
...
void gauss_newton_running_derivatives(
    ...
    const StateVec& x, const ControlVec& u,
    ...
    for (int i = 0; i < MPC_NX; ++i) {
        StateVec xp = x; xp(i) += eps;
        StateVec xm = x; xm(i) -= eps;
        jx.col(i) = (residual_fn(xp, u) - residual_fn(xm, u)) / (2.0 * eps);
    }
    for (int i = 0; i < MPC_NU; ++i) {
        ControlVec up = u; up(i) += eps;
        ControlVec um = u; um(i) -= eps;
...
template <int NR, typename ResidualFn>
void gauss_newton_terminal_derivatives(
    ResidualFn&& residual_fn,
    const StateVec& x,
    StateVec& lx,
    MatXX& lxx
...
CostSample eval_cost_bilinear(
    const CostMapGridView& grid, const GridInfo& info,
    ...
    if (gx < 0 || gy < 0 || gx >= info.width - 1 || gy >= info.height - 1) return s;

    const int x0 = static_cast<int>(std::floor(gx));
    const int y0 = static_cast<int>(std::floor(gy));
    const double dx = gx - x0, dy = gy - y0;

    const double v00 = grid.value_at_clamped(y0, x0);
    const double v01 = grid.value_at_clamped(y0, x0+1);
    const double v10 = grid.value_at_clamped(y0+1, x0);
    const double v11 = grid.value_at_clamped(y0+1, x0+1);

    s.value = (1-dy)*((1-dx)*v00 + dx*v01) + dy*((1-dx)*v10 + dx*v11);
    s.dx = ((1-dy)*(v01-v00) + dy*(v11-v10)) * info.inv_resolution;
    s.dy = ((1-dx)*(v10-v00) + dx*(v11-v01)) * info.inv_resolution;
    return s;
}

DirSample eval_dir_bilinear(
    const DirectionMapGridView& grid, const GridInfo& info,
    double x_map, double y_map
) {
    ...
    const auto v00 = grid.value_at_clamped(y0, x0);
    const auto v01 = grid.value_at_clamped(y0, x0+1);
    const auto v10 = grid.value_at_clamped(y0+1, x0);
    const auto v11 = grid.value_at_clamped(y0+1, x0+1);

    s.value = (1-dy)*((1-dx)*v00 + dx*v01) + dy*((1-dx)*v10 + dx*v11);
    s.J.col(0) = ((1-dy)*(v01-v00) + dy*(v11-v10)) * info.inv_resolution;
    s.J.col(1) = ((1-dx)*(v10-v00) + dx*(v11-v01)) * info.inv_resolution;
    return s;
}
...
FollowProblem::FollowProblem(
    ...
    const MPCParams& params, const CostMapGridView& cost_grid, const GridInfo& cost_info,
...
) : ref_cps_(ref_control_points), p_(params), cost_grid_(cost_grid), cost_info_(cost_info),
    dir_grid_(dir_grid), dir_info_(dir_info), arc_table_(arclength_table),
    remaining_energy_(remaining_energy), rfr_pwr_limit_(rfr_pwr_limit) {}
...
void FollowProblem::dynamics_jacobians(int, const StateVec& x, const ControlVec& u, MatXX& dfx, MatXU& dfu) const {
    ...
    for (int i = 0; i < MPC_NX; ++i) {
        ...
        StateVec xm = x; xm(i) -= eps;
        dfx(ix::PATH_U, i) = (
            advance_u_progress(xp(ix::PATH_U), xp, ref_cps_) -
            advance_u_progress(xm(ix::PATH_U), xm, ref_cps_)
        ) / (2.0 * eps);
    }
...
FollowResidualVec follow_residual_impl(
    ...
    const MPCParams& p,
    const CostMapGridView& cg, const GridInfo& ci,
    const DirectionMapGridView& dg, const GridInfo& di,
...
double FollowProblem::running_cost(int k, const StateVec& x, const ControlVec& u) const {
    ...
    return residual_cost(follow_residual_impl(
        x, u, ref_cps_, arc_table_,
        p_, cost_grid_, cost_info_, dir_grid_, dir_info_,
        rfr_pwr_limit_
    ));
}
...
double FollowProblem::terminal_cost(const StateVec&) const { return 0.0; }
...
StopProblem::StopProblem(
    ...
    cost_grid_(cost_grid), cost_info_(cost_info),
    dir_grid_(dir_grid), dir_info_(dir_info),
    remaining_energy_(remaining_energy), rfr_pwr_limit_(rfr_pwr_limit) {}
...
double StopProblem::running_cost(int k, const StateVec& x, const ControlVec& u) const {
    (void)k;
    return residual_cost(stop_residual_impl(
        x, u, p_, cost_grid_, cost_info_,
        dir_grid_, dir_info_, rfr_pwr_limit_
    ));
}
...
void StopProblem::running_cost_derivatives(
    ...
    (void)k;
    auto residual_fn = [&](const StateVec& xv, const ControlVec& uv) {
        return stop_residual_impl(
            xv, uv, p_, cost_grid_, cost_info_,
            dir_grid_, dir_info_, rfr_pwr_limit_
        );
    };
...
RecoveryProblem::RecoveryProblem(
    const Eigen::Vector2d& goal_map, const MPCParams& params,
    const CostMapGridView& cost_grid, const GridInfo& cost_info,
    const DirectionMapGridView& dir_grid, const GridInfo& dir_info,
    double remaining_energy, double rfr_pwr_limit
) : goal_(goal_map), p_(params),
    cost_grid_(cost_grid), cost_info_(cost_info),
    dir_grid_(dir_grid), dir_info_(dir_info),
    remaining_energy_(remaining_energy), rfr_pwr_limit_(rfr_pwr_limit) {}
...
double RecoveryProblem::running_cost(int k, const StateVec& x, const ControlVec& u) const {
    (void)k;
    return residual_cost(recovery_residual_impl(
        x, u, goal_, p_,
        cost_grid_, cost_info_, dir_grid_, dir_info_,
        rfr_pwr_limit_
    ));
}
...
MPCPrediction rollout_prediction(const ProblemT& prob, const SolverT& solver, const StateVec& x0) {
    ...
    for (int k = 0; k < MPC_HORIZON; ++k) {
        xs_pred[static_cast<size_t>(k + 1)] = prob.dynamics(k, xs_pred[static_cast<size_t>(k)], solver.us[static_cast<size_t>(k)]);
    }

    MPCPrediction pred;
    pred.path_map.reserve(MPC_HORIZON + 1);
    pred.headings.reserve(MPC_HORIZON + 1);
    pred.v_pred.reserve(MPC_HORIZON + 1);
    pred.w_pred.reserve(MPC_HORIZON + 1);
...
void MPCSolver::set_last_cmd(const Eigen::Vector2d& cmd) { last_cmd_ = cmd; }
...
StateVec MPCSolver::make_initial_state(
    ...
    StateVec x0;
    x0(ix::X) = pose.x(); x0(ix::Y) = pose.y(); x0(ix::THETA) = pose.z();
    x0(ix::XH) = x_h_hat_;
    x0(ix::V) = status.x(); x0(ix::W) = status.y();
    x0(ix::DV) = cmd_clamped.x(); x0(ix::DW) = cmd_clamped.y();
    x0(ix::PATH_U) = path_u;
    return x0;
}
...
MPCPrediction MPCSolver::extract_prediction(const StateVec* xs, int n) {
    ...
    pred.path_map.reserve(sz); pred.headings.reserve(sz);
    pred.v_pred.reserve(sz); pred.w_pred.reserve(sz);
    for (int i = 0; i < n; ++i) {
...
std::expected<std::tuple<Eigen::Vector2d, MPCPrediction>, std::string> MPCSolver::follow_path(
    ...
    const double u0 = project_to_spline_u(
        global_path, chassis_pose_map.head<2>(), projection_hint,
    ...
    const Eigen::Vector2d cmd0(
        clamp_prev_cmd(last_cmd_.x(), chassis_status.x(), params_.follow_limits.start_vel_cmd_act_diff_max, params_.follow_limits.acc_max, MPC_DT),
        clamp_prev_cmd(last_cmd_.y(), chassis_status.y(), params_.follow_limits.start_omega_cmd_act_diff_max, params_.follow_limits.alpha_max, MPC_DT)
    );
    ...
    initialize_primal_trajectory(
        follow_solver_, prob, x0, follow_warm_
    );
...
std::expected<std::tuple<Eigen::Vector2d, MPCPrediction>, std::string> MPCSolver::stop(
    ...
    const Eigen::Vector2d cmd0(
        clamp_prev_cmd(last_cmd_.x(), chassis_status.x(), params_.stop_limits.start_vel_cmd_act_diff_max, params_.stop_limits.acc_max, MPC_DT),
        clamp_prev_cmd(last_cmd_.y(), chassis_status.y(), params_.stop_limits.start_omega_cmd_act_diff_max, params_.stop_limits.alpha_max, MPC_DT)
    );
    ...
    initialize_primal_trajectory(
        stop_solver_, prob, x0, stop_warm_
    );
...
std::expected<std::tuple<Eigen::Vector2d, MPCPrediction>, std::string> MPCSolver::recover_to_point(
    ...
    const Eigen::Vector2d cmd0(
        clamp_prev_cmd(last_cmd_.x(), chassis_status.x(), params_.recovery_limits.start_vel_cmd_act_diff_max, params_.recovery_limits.acc_max, MPC_DT),
        clamp_prev_cmd(last_cmd_.y(), chassis_status.y(), params_.recovery_limits.start_omega_cmd_act_diff_max, params_.recovery_limits.alpha_max, MPC_DT)
    );
    ...
    initialize_primal_trajectory(
        recovery_solver_, prob, x0, recovery_warm_
    );
