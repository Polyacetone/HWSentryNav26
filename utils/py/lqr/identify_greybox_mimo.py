...
def generate_kinematic_model_yaml(params: np.ndarray) -> str:
    a11, a12, a21, a22, b1, b2, cf1, cf2, tau_w, cf3 = params[:10]
    xh0 = float(params[10]) if len(params) > 10 else 0.0
    Ad00, _, Ad10, _, _, _, _, _ = _zoh_v_matrices(
        float(a11), float(a12), float(a21), float(a22), float(b1), float(b2), DT)
    obs_target_pole = 0.6
    obs_L = (Ad00 - obs_target_pole) / Ad10 if abs(Ad10) > 1e-10 else 0.0
    lines = [
        "/**:",
        ...
        f"      sgn_eps: {SGN_EPS}",
        "",
        "      # Longitudinal hidden-state model:",
        "      #   x_h_dot = a11*x_h + a12*v + b1*dv",
        "      #   v_dot   = a21*x_h + a22*v + b2*dv + cf1*tanh(v/sgn_eps) + cf2*v*abs(w)",
        f"      a11: {float(a11)}",
        f"      a12: {float(a12)}",
        f"      a21: {float(a21)}",
        f"      a22: {float(a22)}",
        ...
        f"      cf2: {float(cf2)}",
        "",
        "      # Yaw channel: w_dot = (1/tau_w) * (dw - w) - cf3 * tanh(w/sgn_eps)",
        f"      tau_w: {float(tau_w)}",
        f"      cf3: {float(cf3)}",
        "",
        "      # Hidden-state observer: x_h_hat[k+1] = xh_pred + obs_l * (v_meas - v_pred)",
        f"      xh0: {xh0}",
        f"      obs_l: {float(obs_L)}",
    ]
    return "\n".join(lines)
...
def main() -> int:
    ...
    ap.add_argument("--cutoff-hz", type=float, default=8.0, help="Anti-alias lowpass cutoff (Hz) before raw→MPC-rate downsample")
    ap.add_argument("--w-vel", type=float, default=1.0, help="Weight for velocity RMSE term")
    ap.add_argument("--w-pos", type=float, default=1.0, help="Weight for displacement integral term")
    args = ap.parse_args()
    ...
    yaml_path = out_dir / "kinematic_model.yaml"
    yaml_path.write_text(generate_kinematic_model_yaml(final_params) + "\n", encoding="utf-8")
