...
def save_height_map_cv(height_map, output_path):
    ...
    norm_height_map[valid_mask] = ((height_map[valid_mask] - min_height) * 200).astype(np.uint8)
    print(f"高度归一化: 最小高度={min_height:.3f}米, 像素值单位=0.5cm")
...
if __name__ == "__main__":
    pcd_path="RMUL2026.pcd"
    cloud = o3d.io.read_point_cloud(pcd_path)
    cloud = crop_cloud_by_bounds(cloud, min_bound=(0, 0, -0.1), max_bound=(1e3, 1e3, 1.0))
