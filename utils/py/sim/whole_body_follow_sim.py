"""
轮腿平衡机器人ROS2仿真器
基于MATLAB模型移植的Python仿真程序，支持ROS2话题交互

状态向量 (10维):
    x = [s, ds, phi, dphi, theta_l_l, dtheta_l_l, theta_l_r, dtheta_l_r, theta_b, dtheta_b]
    - s: 自然坐标系下机器人水平方向移动距离
    - phi: 偏航角 (yaw)
    - theta_l_l, theta_l_r: 左右腿倾斜角
    - theta_b: 机体俯仰角 (pitch)
...
订阅话题:
    /path_follower/chassis_cmd (ChassisCmd) - 底盘控制指令

发布话题:
    /serial_bridge/joint_state (JointState) - 云台关节状态
    /small_glim/odometry (Odometry) - 里程计
    /serial_bridge/imu (Imu) - IMU数据
    /serial_bridge/chassis_status (ChassisStatus) - 底盘状态
"""
...
class SimConfig:
    ...
    # 控制指令限幅
    MAX_VELOCITY: float = 2.8           # 最大线速度 (m/s)
    MAX_PALSTANCE: float = 7.2         # 最大角速度 (rad/s)
    MAX_ACCEL: float = 2.5              # 最大线加速度 (m/s^2)
    MAX_ANG_ACCEL: float = 12.0         # 最大角加速度 (rad/s^2)
    MAX_V_W_PRODUCT: float = 3.5        # 最大速度角速度积 (m*rad/s^2)
    
    # 小陀螺参数
    SPIN_FAST_OMEGA: float = 7.2       # 快速小陀螺角速度 (rad/s)
    SPIN_SLOW_OMEGA: float = 3.0        # 慢速小陀螺角速度 (rad/s)
    
    # 里程计时间偏移 (模拟里程计延迟)
    ODOM_TIME_OFFSET_SEC: float = 0.12
...
class WheelLegSimulationNode(Node):
    """轮腿平衡机器人ROS2仿真节点"""
    
    def __init__(self):
        super().__init__('wheel_leg_simulation_node')
        ...
        # 计算时间参数
        self._lqr_dt = 1.0 / self.config.LQR_FREQ
        self._publish_dt = 1.0 / self.config.PUBLISH_FREQ
        self._lqr_steps_per_publish = int(self.config.LQR_FREQ / self.config.PUBLISH_FREQ)
        
        self._cmd_timeout = Duration(seconds=self.config.CMD_TIMEOUT_SEC)
        self._odom_time_offset = Duration(seconds=self.config.ODOM_TIME_OFFSET_SEC)
        
        # 初始化动力学模型
        self.dynamics = WheelLegDynamics(self.robot_params)
        self.dynamics.reset(
            x=self.config.INIT_X,
            y=self.config.INIT_Y,
            theta=self.config.INIT_THETA
        )
        ...
        self.cloud_pub = self.create_publisher(PointCloud2, '/small_glim/registered_cloud', 2)
        self.tf_broadcaster = TransformBroadcaster(self)
        
        # 订阅器
        self.create_subscription(ChassisCmd, '/path_follower/chassis_cmd', self._cmd_callback, 2)
        
        # 定时器 (以发布频率运行，每次执行多步LQR)
        self.timer = self.create_timer(self._publish_dt, self._timer_callback)
        
        self.get_logger().info(
            f"Wheel-Leg Simulation Node Started\n"
            f"  LQR Freq: {self.config.LQR_FREQ} Hz\n"
            f"  Publish Freq: {self.config.PUBLISH_FREQ} Hz\n"
            f"  LQR steps per publish: {self._lqr_steps_per_publish}\n"
            f"  Initial pose: ({self.config.INIT_X}, {self.config.INIT_Y}, {self.config.INIT_THETA})"
        )
    
    def _cmd_callback(self, msg: ChassisCmd):
        """处理底盘控制指令"""
        now = self.get_clock().now()
        
        # 保存上一次的指令用于计算加速度
        self._prev_cmd_v = self._limited_v
        self._prev_cmd_w = self._limited_w
        
        # 获取新指令
        self._slow_spin = bool(msg.slow_spin)
        self._fast_spin = bool(msg.fast_spin)
        
        if self._slow_spin or self._fast_spin:
            # 小陀螺模式
            self._cmd_v = 0.0
            spin_omega = self.config.SPIN_FAST_OMEGA if self._fast_spin else self.config.SPIN_SLOW_OMEGA
            # 保持当前旋转方向
    ...
    def _apply_limits(self, v: float, w: float, 
                      prev_v: float, prev_w: float, 
        ...
        if dt > 0:
            ...
            max_dw = self.config.MAX_ANG_ACCEL * dt
        ...
        # 3. 限制速度角速度积 (|v * w| <= MAX_V_W_PRODUCT)
        vw_product = abs(v * w)
        if vw_product > self.config.MAX_V_W_PRODUCT:
            scale = self.config.MAX_V_W_PRODUCT / vw_product
            # 等比例缩放
            v *= math.sqrt(scale)
            w *= math.sqrt(scale)
    ...
    def _publish_messages(self, now: rclpy.time.Time):
        """发布所有ROS2消息"""
        ...
        status_msg.velocity = float(self.dynamics.velocity)
        status_msg.palstance = float(self.dynamics.palstance)
