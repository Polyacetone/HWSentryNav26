...
ExecutorOutput PathExecutor::update(const ExecutorInput& input) {
    uint8_t leg_mode = input.chassis_leg_mode;
    ...
    const ChassisControlState chassis_control_state = classify_chassis_control_state(leg_mode, input.comp_stage);
    ...
    const bool has_path = input.active_path != nullptr;

    // ── path 绑定切换（新的不可变包 → 重置台阶/进度状态）──
    if (input.active_path != bound_path_) {
        bound_path_ = input.active_path;
    ...
    if (prev_state == MotionState::HAZARD_RECOVERY
        && input.masked_global_cost_map && input.masked_direction_map) {
    ...
    if (!command_blocked && has_path) {
        if (prev_state == MotionState::FOLLOW) {
            no_progress_detected = progress_monitor_.update_and_check_no_progress(
                current_u, params_.follow_no_progress_guard, MotionState::FOLLOW, prev_state, input.stamp
            );
        } else if (prev_state == MotionState::STEPPING) {
            no_progress_detected = progress_monitor_.update_and_check_no_progress(
                current_u, params_.stepping_no_progress_guard, MotionState::STEPPING, prev_state, input.stamp
            );
        }
    }

    const bool dist_reached = has_path
        && ((input.chassis_pose_map.head<2>() - bound_path_->spline.position(1.0)).norm() < params_.stop_threshold_dist);
    const bool u_reached = has_path && (current_u > params_.stop_threshold_u);

    // ── 组装 FSM 输入 ──
    FsmInput fsm_input;
    fsm_input.has_path = has_path;
    fsm_input.has_hold_goal = input.hold_goal.has_value();
    ...
    fsm_input.is_hazard = !command_blocked
        ...
        && safety_monitor_.compute_is_hazard(
            *input.masked_global_cost_map, *input.masked_direction_map, input.chassis_pose_map.head<2>()
        );
    fsm_input.is_stuck = !command_blocked && safety_monitor_.check_stuck(
        input.chassis_pose_map.head<2>(), last_cmd_.x(), input.stamp
    );
    fsm_input.is_recovery_safe = !command_blocked
        ...
        && safety_monitor_.check_recovery_safe(
            *input.final_cost_map, *input.masked_direction_map, input.chassis_pose_map.head<2>(), input.stamp
        );
    fsm_input.chassis_pos_map = input.chassis_pose_map.head<2>();
...
ExecutorOutput PathExecutor::execute_follow(const ExecutorInput& input) {
    ExecutorOutput out;
    if (!input.active_path || !input.final_cost_map || !input.masked_global_cost_map || !input.masked_direction_map) {
        return out;
    }
    ...
    const auto result = mpc_controller_->solve_follow(
        path, input.chassis_pose_map, input.chassis_state,
...
ExecutorOutput PathExecutor::execute_stop(const ExecutorInput& input) {
    ...
    const auto result = mpc_controller_->solve_stop(
        input.chassis_pose_map, input.chassis_state, *input.final_cost_map
    );
    if (!result) {
...
ExecutorOutput PathExecutor::execute_recovery(const ExecutorInput& input) {
    ...
    const auto result = mpc_controller_->solve_hold(
        *recovery_goal, input.chassis_pose_map, input.chassis_state, *input.final_cost_map
    );
    if (!result) {
...
ExecutorOutput PathExecutor::execute_fixed(const ExecutorInput& input) {
    ExecutorOutput out;
    if (!input.final_cost_map || !input.masked_direction_map || !input.hold_goal) return out;

    const SolveTimer timer;
    const auto result = mpc_controller_->solve_hold(
        *input.hold_goal, input.chassis_pose_map, input.chassis_state, *input.final_cost_map
    );
    if (!result) {
