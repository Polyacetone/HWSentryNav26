...
void AsyncGlobalMapping::insert_imu(
    const double stamp,
...
void AsyncGlobalMapping::save(const std::string& path) {
    logger::info("global_mapping", "saving to {}...", path);
...
std::vector<Eigen::Vector4d> AsyncGlobalMapping::export_points() {
    std::lock_guard<std::mutex> lock(global_mapping_mutex);
    logger::info("global_mapping", "exporting points");
    ...
    while (!kill_switch) {
        ...
        if (imu_frames.empty() && submaps.empty()) {
            ...
            if (request_to_optimize ||
                std::chrono::high_resolution_clock::now() - last_optimization_time > std::chrono::seconds(optimization_interval)) {
                std::lock_guard<std::mutex> lock(global_mapping_mutex);
                request_to_optimize = false;
                global_mapping->optimize();
