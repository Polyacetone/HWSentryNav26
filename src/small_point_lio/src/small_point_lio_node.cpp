...
namespace small_point_lio {
    ...
    SmallPointLioNode::SmallPointLioNode(const rclcpp::NodeOptions &options)
        ...
        small_point_lio->set_odometry_callback([this](const common::Odometry &odometry) {
            ...
            try {
                base_link_to_livox_frame_transform = tf_buffer->lookupTransform("livox_frame", "base", odometry_msg.header.stamp);
            } catch (tf2::TransformException &ex) {
                RCLCPP_ERROR(rclcpp::get_logger("small_point_lio"), "Failed to lookup transform from base to livox_frame: %s", ex.what());
                return;
            }
