...
private:
    ...
    void plan_and_publish_to_goal(const Eigen::Vector2d& goal_map, bool fixed);
...
GlobalPlannerNode::GlobalPlannerNode(const rclcpp::NodeOptions& options): Node("global_planner", options) {
...
bool GlobalPlannerNode::is_map_point_feasible(const CostMap& cost_map, const DirectionMap& direction_map, const Eigen::Vector2d& map_pt) const {
...
Eigen::Vector2d GlobalPlannerNode::adjust_reachable_start_on_segment(const Eigen::Vector2d& from_map, const Eigen::Vector2d& to_map) const {
    ...
    for (int i = 0; i <= n; ++i) {
        ...
        if (!is_map_point_feasible(*global_cost_map_, *global_direction_map_, pt)) break;
        last_feasible = pt;
    }
    return last_feasible;
}

Eigen::Vector2d GlobalPlannerNode::predict_start_map(const Eigen::Vector2d& current_map) const {
...
void GlobalPlannerNode::goal_callback(const interfaces::msg::NavGoal::SharedPtr msg) {
    plan_and_publish_to_goal(Eigen::Vector2d(msg->x, msg->y), msg->fixed);
}

void GlobalPlannerNode::plan_and_publish_to_goal(const Eigen::Vector2d& goal_map, bool fixed) {
    if (!global_cost_map_ || !global_direction_map_) {
        RCLCPP_WARN(get_logger(), "Map not ready yet!");
        return;
    }
    ...
    RCLCPP_INFO(
        get_logger(), "Planning: Src(%.2f, %.2f) -> Dst(%.2f, %.2f) %s",
...
nav_msgs::msg::Path GlobalPlannerNode::path_to_nav_msg(const std::vector<Eigen::Vector2d>& path) const {
...
RCLCPP_COMPONENTS_REGISTER_NODE(global_planner::GlobalPlannerNode)
