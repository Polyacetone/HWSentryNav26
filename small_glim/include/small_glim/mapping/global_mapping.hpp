...
public:
    explicit GlobalMappingParams(const Config::Ptr config);
...
class GlobalMapping {
public:
    explicit GlobalMapping(const Config::Ptr config);
    ...
    void find_overlapping_submaps(double min_overlap);
    void optimize();
...
private:
    ...
    std::pair<gtsam::NonlinearFactorGraph, gtsam::Values> recover_graph(
...
private:
    std::unique_ptr<GlobalMappingParams> params;
