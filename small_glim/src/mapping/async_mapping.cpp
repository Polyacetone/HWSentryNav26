...
bool AsyncMapping::is_keyframe(const EstimationFrame& frame) const {
    const Eigen::Isometry3d T_world_lidar = frame.T_world_lidar;

    if (!has_last_keyframe_) {
        return true;
    }

    const Eigen::Isometry3d T_last_curr = last_keyframe_T_world_lidar_.inverse() * T_world_lidar;
...
void AsyncMapping::accept_keyframe(const EstimationFrame& frame) {
    ...
    last_keyframe_T_world_lidar_ = T_world_lidar;
    keyframe_count_++;
...
void AsyncMapping::save_keyframe_raw(const EstimationFrame& frame) {
    ...
    for (int i = 0; i < frame.frame->size(); i++) {
        ...
        cloud.push_back(pcl::PointXYZ(static_cast<float>(p.x()), static_cast<float>(p.y()), static_cast<float>(p.z())));
    }
...
void AsyncMapping::append_pose(const Eigen::Isometry3d& T_world_lidar) {
    poses_ofs_ << convert_to_string(T_world_lidar) << "\n";
}
...
void AsyncMapping::integrate_into_map(const EstimationFrame& frame) {
    ...
    for (int i = 0; i < frame.frame->size(); i++) {
        ...
        p = T_world_lidar * p;
        map_cloud_->push_back(pcl::PointXYZ(static_cast<float>(p.x()), static_cast<float>(p.y()), static_cast<float>(p.z())));
    }
}
...
void AsyncMapping::downsample_map_if_needed() {
    ...
    if (params_.downsample_every_n_keyframes <= 0) {
        return;
    }
    if (keyframes_since_downsample_ < params_.downsample_every_n_keyframes) {
        return;
    }
    if (params_.map_voxel_leaf_size <= 0.0) {
        return;
    }
    ...
    vg.setLeafSize(
        static_cast<float>(params_.map_voxel_leaf_size),
        static_cast<float>(params_.map_voxel_leaf_size),
        static_cast<float>(params_.map_voxel_leaf_size)
    );
